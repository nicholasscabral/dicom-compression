...
def convert_dicom_to_pca_single(dicom_path, variance_percentage):
    ...
    try:
        ...
        # Porcentagem de variância a ser mantida
        variance_ratio = variance_percentage / 100.0
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            "Uso: python script.py /caminho/para/arquivo_dicom porcentagem_de_variancia"
        )
        sys.exit(1)

    dicom_path = sys.argv[1]
    variance_percentage = float(sys.argv[2])  # Porcentagem de variância a ser utilizada

    # Chama a função de conversão
    convert_dicom_to_pca_single(dicom_path, variance_percentage)
